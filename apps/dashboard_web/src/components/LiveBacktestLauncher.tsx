...
export function LiveBacktestLauncher({ onRunCreated }: Props) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setCatalog(next);
      setStrategyId(
        current => current || next.strategies[0]?.package_id || ""
      );
    } catch (reason) {
  ...
  const selected = useMemo(
    () =>
      catalog?.strategies.find(
        item => item.package_id === strategyId
      ) ?? null,
    [catalog, strategyId]
  );
      ...
      <section className="live-launch-card">
        <div className="live-launch-heading">
          <div>
            ...
            <p>
              The engine stays online. Strategy packages are discovered from
              the mounted strategies directory and each closed candle is
              delivered to an isolated strategy process.
            </p>
          </div>
        ...
        <div className="live-form-grid">
          ...
          <label className="live-check">
            <input
              ...
              checked={runUntilEnd}
              onChange={event =>
                setRunUntilEnd(event.target.checked)
              }
            />

            <span>Run until end of available data</span>
          </label>

          <label className="live-field">
            <span>Maximum close batches</span>

            <input
              type="number"
              min="1"
              value={maxBatches}
              disabled={runUntilEnd}
              onChange={event => {
          ...
          <label className="live-check">
            ...
            <span>Start paused for manual candle stepping</span>
          </label>
        </div>
        ...
        <div className="live-run-list">
          {catalog?.runs.length ? (
            catalog.runs.map(run => (
              <button
                key={run.run_id}
                onClick={() => onRunCreated(run.run_id)}
              >
                <strong>{run.descriptor.name}</strong>

                <span>
                  {run.status} · {run.processed_close_batches} batches ·{" "}
                  {(Number(run.progress) * 100).toFixed(2)}%
                </span>
              </button>
            ))
          ) : (
