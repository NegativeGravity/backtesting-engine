...
it("compacts indicator points in chunks instead of replacing the whole series every point", async () => {
  ...
  const state = emptyChartState();
  for (let timeNs = 1; timeNs <= 8_001; timeNs += 1) {
  ...
  expect(state.points.get("close")).toHaveLength(6_000);
  expect(state.points.get("close")?.[0]?.timeNs).toBe(2_002);

  applyChartCommand(state, {
    command_type: "append_series_point",
    series_id: "close",
    point: { point_type: "scalar", time_ns: 8_002, value: 8_002 }
  });
  expect(state.points.get("close")).toHaveLength(6_001);
});
