...
it("keeps open position time dynamic instead of rebuilding it for every candle", () => {
  const drawings = buildBrokerTradeDrawings({
    symbol: "XAUUSD",
    trades: [],
    positions: [{
      ...
      status: "open",
      current_price_ticks: 250100,
      stop_loss_ticks: 249000,
      take_profit_ticks: 251000,
      unrealized_pnl: "10",
      realized_pnl: "0",
      opened_time_ns: 1_000_000_000
    }],
    latestTimeNs: 9_000_000_000
  });

  expect(drawings[0]?.payload.status).toBe("open");
  expect(drawings[0]?.payload.exit_time_ns).toBeNull();
});
