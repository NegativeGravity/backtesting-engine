...
class YjBoxBreakoutStrategy(Strategy):
    ...
    def on_bar(self, context: StrategyContext, bar: Bar) -> None:
        ...
        if bar.symbol != config.symbol or bar.timeframe is not config.signal_timeframe:
            return
        if self._awaiting_reversal is not None and not context.portfolio.positions(config.symbol):
    ...
    def _finalize_box(self, context: StrategyContext, trade_date: date) -> None:
        ...
        expected_minutes = tuple(range(config.box_start_minute, config.box_end_minute, 15))
        missing_minutes = tuple(minute for minute in expected_minutes if minute not in day_bars)
        if missing_minutes:
    ...
    def _submit_breakout_pair(self, context: StrategyContext, box: BoxState) -> None:
        ...
        target_distance = int(
            (Decimal(distance) * config.reward_risk_ratio).to_integral_value(rounding=ROUND_HALF_UP)
        )
    ...
    def _position_closed(
        ...
        r_text = "" if trade.realized_r_multiple is None else f" | {trade.realized_r_multiple:.2f}R"
        context.chart.risk_reward(
        ...
        context.log.info(
            ...
            r_multiple=(
                None if trade.realized_r_multiple is None else float(trade.realized_r_multiple)
            ),
        )
    ...
    def _draw_box(self, context: StrategyContext, box: BoxState) -> None:
        ...
        context.chart.rectangle(
            ...
            border_width=2,
            label=(f"TEHRAN 01:30–03:30 BOX | High {box.high_ticks} | Low {box.low_ticks}"),
            z_index=5,
        )
    ...
    def _handle_reversal_rejection(
        ...
        payload = {key: value for key, value in event.payload.items() if key in Order.model_fields}
        if not payload:
            return
    ...
    def _prune_state(self, current_date: date) -> None:
        ...
        self._chain_ids_by_date = {
            day: chain_id for day, chain_id in self._chain_ids_by_date.items() if day >= cutoff
        }
