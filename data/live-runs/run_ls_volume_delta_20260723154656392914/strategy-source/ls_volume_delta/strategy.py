...
class LsVolumeDeltaStrategy(Strategy):
    ...
    def __init__(self, parameters: dict[str, JsonValue]) -> None:
        ...
        self._risk = RiskGovernor(
            ...
            maximum_primary_take_profits_per_day=(config.maximum_primary_take_profits_per_day),
    ...
    def on_bar(self, context: StrategyContext, bar: Bar) -> None:
        ...
        if bar.timeframe is self.config.delta_source_timeframe:
            ...
            self._delta.prune(candle.close_time_ns - 2 * 24 * 60 * 60 * NANOSECONDS_PER_SECOND)
            return
        ...
        in_session = self.config.session_start_minute <= minute < self.config.session_end_minute
        ...
        session_high_before, session_low_before = self._session.snapshot_extremes()
    ...
    def _submit_primary(
        ...
        *,
        ...
        chain_id = f"ls-{trade_date.isoformat()}-{self._chain_sequence:05d}"
        tags = {
            ...
            "hunted_structure_ticks": (
                "" if signal.hunted_structure_ticks is None else str(signal.hunted_structure_ticks)
            ),
            ...
            EXECUTION_REWARD_RISK_TAG: str(self.config.primary_reward_risk),
            ...
            tags.update(
                {
                    ...
                    STOP_AND_REVERSE_STOP_TICKS_TAG: str(signal.cover_stop_ticks),
                    STOP_AND_REVERSE_REWARD_RISK_TAG: str(self.config.cover_reward_risk),
    ...
    def _position_opened(
        ...
        if position.symbol != self.config.symbol:
            return
        tags = self._validated_entry_tags(
            getattr(position, "entry_tags", {}) or {},
        ...
        try:
            ...
            raise RuntimeError(
                f"LS position contains an invalid trade_date entry tag: {raw_date!r}"
            ) from exc
        ...
        if leg == 2:
            context.chart.delete(f"ls.cover.entry.{signal.candle.open_time_ns}")

        if position.stop_loss_ticks is not None and position.take_profit_ticks is not None:
    ...
    def _position_closed(
        ...
        if leg == 1 and take_profit and visual is not None:
            context.chart.delete(f"ls.cover.entry.{visual.signal.candle.open_time_ns}")
        ...
        context.chart.marker(
            ...
            text=(f"{'TP' if take_profit else 'SL'} {realized_r:+.2f}R"),
            z_index=32,
        )
        context.log.info(
            "ls_trade_closed",
            ...
            trade_date=(
                visual.trade_date.isoformat() if visual is not None else tags.get("trade_date", "")
            ),
            **self._risk.status(),
        )
    ...
    def _draw_signal(
        ...
        *,
        ...
        context.chart.marker(
            ...
            (ChartMarkerShape.ARROW_UP if long_signal else ChartMarkerShape.ARROW_DOWN),
            (ChartMarkerPosition.BELOW_BAR if long_signal else ChartMarkerPosition.ABOVE_BAR),
            "#20C997" if long_signal else "#FF6B6B",
            price_ticks=(signal.candle.low_ticks if long_signal else signal.candle.high_ticks),
            text=(f"{self._setup_label(signal.setup_kind)} Δ{signal.volume_delta:+d}"),
            z_index=15,
        )
        if signal.hunted_structure_ticks is not None and signal.hunted_structure_id is not None:
            context.chart.trend_line(
                f"ls.hunt.{signal.candle.open_time_ns}",
                self.config.symbol,
                self.config.signal_timeframe,
                signal.hunted_structure_time_ns or signal.candle.open_time_ns,
        ...
        if cover_enabled:
            context.chart.trend_line(
                ...
                signal.candle.close_time_ns,
                signal.stop_ticks,
                self._session_end_ns(self._local_datetime(signal.candle.open_time_ns).date()),
                signal.stop_ticks,
                color="#FCC419",
                width=1,
    ...
    def _draw_raw_confirmation(
        ...
        *,
        ...
        context.chart.marker(
            ...
            candle.open_time_ns,
            ChartMarkerShape.SQUARE,
            (ChartMarkerPosition.BELOW_BAR if long_confirmed else ChartMarkerPosition.ABOVE_BAR),
            "#74C0FC",
            price_ticks=(candle.low_ticks if long_confirmed else candle.high_ticks),
            text=f"LS Δ{delta:+d}",
            z_index=6,
        )
    ...
    def _signal_from_tags(
        ...
        direction = Direction.LONG if position.side is PositionSide.LONG else Direction.SHORT
        candle = Candle(
            open_time_ns=int(tags.get("signal_open_time_ns", position.opened_time_ns)),
            close_time_ns=int(tags.get("signal_close_time_ns", position.opened_time_ns + 1)),
            open_ticks=int(
                tags.get(
                    "signal_open_ticks",
                    position.average_entry_price_ticks,
        ...
        return Signal(
            direction=direction,
            setup_kind=SetupKind(tags.get("setup_kind", SetupKind.LS.value)),
            candle=candle,
            ...
            stop_ticks=(
                ...
                else (candle.low_ticks if direction is Direction.LONG else candle.high_ticks)
            ),
            cover_stop_ticks=(
                candle.body_high_ticks if direction is Direction.LONG else candle.body_low_ticks
            ),
            hunted_structure_id=tags.get("hunted_structure_id") or None,
            hunted_structure_ticks=(
                int(tags["hunted_structure_ticks"]) if tags.get("hunted_structure_ticks") else None
            ),
            hunted_structure_time_ns=(
    ...
    def _validated_entry_tags(
        ...
        *,
        ...
        if tags.get("strategy") != "ls_volume_delta":
            ...
            raise RuntimeError(
                ...
                "OrderRequest.tags must be preserved through PositionState, "
                "Position and Trade."
            )
        missing = sorted(key for key in required if not tags.get(key))
        if missing:
            raise RuntimeError(
                f"LS broker metadata is incomplete for {event_name} {object_id}: missing={missing}"
            )
        return tags
    ...
    def _session_end_ns(self, trade_date: date) -> int:
        ...
        return int(end_local.astimezone(UTC).timestamp() * NANOSECONDS_PER_SECOND)
    ...
    def _session_label(self) -> str:
        ...
        return f"LS SESSION {start_hour:02d}:{start_minute:02d}-{end_hour:02d}:{end_minute:02d}"
