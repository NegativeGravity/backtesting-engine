...
def resolve_reverse_chain_id(
    ...
    *,
    ...
    if client_order_id:
        matches = [chain_id for chain_id in known if chain_id and chain_id in client_order_id]
        if len(matches) == 1:
            return matches[0]
...
class ClockAlignedM2Delta:
    ...
    def push_m1(self, candle: Candle) -> M2VolumeBar | None:
        ...
        selected = [
            item
            for item in bucket
            if item.open_time_ns >= bucket_start and item.close_time_ns <= bucket_end
        ]
    ...
    def prune(self, before_time_ns: int) -> None:
        self._completed = [item for item in self._completed if item.close_time_ns >= before_time_ns]
        self._pending = {
            key: value
...
class StructureLevel:
    ...
    def strict_hunt(self, candle: Candle) -> bool:
        ...
        return (
            candle.low_ticks < self.wick_ticks and candle.low_ticks < self.intervening_extreme_ticks
        )
...
class SessionState:
    ...
    def include(self, candle: Candle) -> None:
        ...
        self.low_ticks = (
            candle.low_ticks if self.low_ticks is None else min(self.low_ticks, candle.low_ticks)
        )
        self.first_time_ns = (
            candle.open_time_ns
        ...
        if long_setup != short_setup:
            if long_setup:
                signal = Signal(
                    ...
                    hunted_structure_id=(long_level.structure_id if long_ls_setup else None),
                    hunted_structure_ticks=(long_level.wick_ticks if long_ls_setup else None),
                    hunted_structure_time_ns=(long_level.formed_time_ns if long_ls_setup else None),
                )
            else:
                signal = Signal(
                    ...
                    hunted_structure_id=(short_level.structure_id if short_ls_setup else None),
                    hunted_structure_ticks=(short_level.wick_ticks if short_ls_setup else None),
...
class RiskGovernor:
    ...
    def roll(self, trade_date: date) -> None:
        if self.month is None or (self.month.year, self.month.month) != (
            trade_date.year,
            trade_date.month,
        ):
    ...
    def permission(self, trade_date: date) -> EntryPermission:
        ...
        if trade_date.day < 15 and self.month.realized_r >= self.monthly_pause_target_r:
        ...
        if self.day.primary_take_profits >= self.maximum_primary_take_profits_per_day:
            self.day.halted = True
    ...
    def status(self) -> dict[str, str | int | bool]:
        return {
            ...
            "monthly_r": str(self.month.realized_r if self.month else Decimal("0")),
